...
pub use engine::{layout_document, LayoutOptions};
...
/// Re-run layout and compare page object ids against the cache.
pub fn verify_render_cache(doc: &Document) -> Result<RecomputeOutcome> {
    ...
    if cache.engine_name != ENGINE_NAME || cache.engine_version != ENGINE_VERSION {
