...
- [x] **2i. Shaped & complex scripts** — **engine 1.4 shipped (v0.14,
      ...
      **`glyphs`** display op (format 0.4): positioned glyphs in visual
      ...
      pinned shaper) — the 1.0–1.3 golden hashes are unchanged.
      **Engine 1.5 shipped (v0.15,
      [docs/LAYOUT-1.5.md](docs/LAYOUT-1.5.md))**: the remaining major
      Brahmic scripts (Bengali, Gurmukhi, Gujarati, Oriya, Tamil, Telugu,
      Kannada, Malayalam, Sinhala) shaped by the same pinned `rustybuzz`,
      and **UAX #9 bidi mirroring** of `Bidi_Mirrored` characters in RTL
      runs (an opening `(` in a Hebrew clause is drawn as `)` while the
      ...
      **Engine 1.9 shipped (v0.19,
      [docs/LAYOUT-1.9.md](docs/LAYOUT-1.9.md))**: the remaining complex
      scripts — **Tibetan** (tsheg line breaking), **Khmer + Myanmar**
      (dictionary line breaking over pinned ICU word lists), **Ethiopic**
      — shaped by the same pinned `rustybuzz`, plus **CJK
